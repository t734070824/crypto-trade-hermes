...
- `cronjob create/update/run/list` and background scheduler activity may rewrite scheduler runtime fields (`repeat.completed`, `next_run_at`, `last_run_at`, `last_status`, root `updated_at`) while you are working, including after a commit/push has already succeeded. Treat unrelated changes to pre-existing jobs as runtime noise; normalize or revert them before staging unless the user explicitly asked to preserve the new runtime state.
- Before each commit, inspect `git status --short` and `git diff -- cron/jobs.json`; if only scheduler timestamps/counters changed, run `git checkout -- cron/jobs.json` before staging. Repeat this check again after push, because the scheduler may tick during commit/push.
