...
- Current startup signed-testnet scope is multi-symbol but still conservative unless the user explicitly changes it: run a BTC group (`BTCUSDT`, `--risk-unit 0.001`, `--account-risk-fraction 0.003`, `--target-leverage 2`, `--testnet-max-order-count 3`) plus an Alt group (`ETHUSDT,SOLUSDT,BNBUSDT`, `--risk-unit 0.1`, `--account-risk-fraction 0.003`, `--target-leverage 2`, `--testnet-max-order-count 6`), both on `--interval 1h` with `--base-url https://testnet.binancefuture.com`, `--testnet-max-order-notional 200`, `--testnet-max-symbol-exposure 70`, `--testnet-max-daily-loss 10`, account sync, lifecycle tracking, open-algo TP/SL reconciliation, and testnet-only endpoint guard.
